...
class RabitQState:
    ...
    def _ensure_device(self, tensor_dict: dict, layer_id: int, device: torch.device) -> torch.Tensor:
        """Move a per-layer tensor to target device if needed, caching the result."""
        t = tensor_dict[layer_id]
        if t.device != device:
            t = t.to(device)
            tensor_dict[layer_id] = t
        return t

    def _get_rotation(self, layer_id: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
        """Get rotation matrix, moving to target device/dtype only if needed."""
        P = self.rotation_matrices[layer_id]
        if P.device != device or P.dtype != dtype:
            P = P.to(device=device, dtype=dtype)
            # Cache on this device to avoid repeated transfers
            self.rotation_matrices[layer_id] = P
        return P

    def rotation(self, x: torch.Tensor, layer_id: int) -> torch.Tensor:
        """Apply rotation: x @ P"""
        P = self._get_rotation(layer_id, x.device, x.dtype)
        return x @ P
    ...
    def train(self, query_states: torch.Tensor, key_states: torch.Tensor, layer_id: int):
        ...
        if layer_id not in self.is_trained or not self.is_trained[layer_id]:
            ...
            if layer_id not in self.rotation_matrices:
                # Create on the layer's device directly to avoid cross-device copy
            ...
            # Compute per-head centroids: [num_heads, head_dim] - stay on layer's GPU
    ...
    def quantize_keys_batch(self, key_states: torch.Tensor, layer_id: int, start_idx: int, end_idx: int):
        ...
        # Extract the batch to quantize and ensure contiguous
        keys_to_quantize = key_states[:, :, start_idx:end_idx, :].contiguous()  # [bs, num_heads, batch_size, head_dim]

        bs, num_heads, batch_size, head_dim = keys_to_quantize.shape
        device = keys_to_quantize.device

        # Use layer-specific per-head centroids, move to device if needed (lazy)
        k_centroid = self._ensure_device(self.k_centroids, layer_id, device)
        q_centroid = self._ensure_device(self.q_centroids, layer_id, device)
        ...
        # Store quantized data on GPU (same device as the layer) for speed
    ...
    def distances(self, query_states: torch.Tensor, layer_id: int, quantized_seq_len: int):
        ...
        # Use layer-specific per-head centroids (lazy device transfer)
        q_centroid = self._ensure_device(self.q_centroids, layer_id, device)
        k_centroid = self._ensure_device(self.k_centroids, layer_id, device)
        cq_dot_ck = self._ensure_device(self.cq_dot_ck, layer_id, device)

        # Concatenate all quantized data (already on layer's GPU, no transfer needed normally)
        codes = torch.cat(self.quantized_data[layer_id]['codes'], dim=2)
        norms = torch.cat(self.quantized_data[layer_id]['norms'], dim=2)
        o_Kbar = torch.cat(self.quantized_data[layer_id]['o_Obar'], dim=2)
        cq_dot_kr = torch.cat(self.quantized_data[layer_id]['cq_dot_kr'], dim=2)

        # Safety: move to query device if somehow different (rare in normal pipeline parallelism)
        if codes.device != device:
            codes = codes.to(device)
            norms = norms.to(device)
            o_Kbar = o_Kbar.to(device)
            cq_dot_kr = cq_dot_kr.to(device)
