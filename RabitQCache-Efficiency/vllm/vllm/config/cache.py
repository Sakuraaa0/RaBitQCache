...
class CacheConfig:
    ...
    def compute_hash(self) -> str:
        ...
        factors.append(self.enable_rabitq)
        factors.append(self.rabitq_b_q)
        factors.append(self.rabitq_topk)
        factors.append(self.rabitq_topp)
    ...
    def _verify_args(self) -> Self:
        ...
        if self.enable_rabitq and self.rabitq_b_q is not None:
            if self.rabitq_b_q <= 0:
                raise ValueError("rabitq_b_q must be positive when RabitQ "
                                 "backend is enabled.")

        if self.enable_rabitq and self.rabitq_topk is not None:
            if self.rabitq_topk <= 0:
                raise ValueError("rabitq_topk must be positive when RabitQ "
                                 "backend is enabled.")

        if self.enable_rabitq and self.rabitq_topp is not None:
            if not (0.0 < self.rabitq_topp <= 1.0):
                raise ValueError("rabitq_topp must be in (0.0, 1.0] when RabitQ "
                                 "backend is enabled.")
    ...
    enable_rabitq: bool = False
    """Enable RabitQ quantized KV cache backend."""
    rabitq_b_q: Optional[int] = None
    """Bit width used to quantize rotated queries in RabitQ backend."""
    rabitq_topk: Optional[int] = None
    """Top-K value for RabitQ attention (default: 128)."""
    rabitq_topp: Optional[float] = None
    """Top-P value for RabitQ attention (0.0-1.0). If set, uses top-p instead of top-k."""
