...
class Attention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        if cache_config is not None and cache_config.enable_rabitq \
                and envs.VLLM_USE_V1:
            extra_impl_args.setdefault("rabitq_b_q", cache_config.rabitq_b_q)
            extra_impl_args.setdefault("rabitq_topk", cache_config.rabitq_topk)
            extra_impl_args.setdefault("rabitq_topp", cache_config.rabitq_topp)

        if attn_backend is None:
            if (cache_config is not None and cache_config.enable_rabitq
                    and envs.VLLM_USE_V1):
                from vllm.v1.attention.backends.rabitq import (
                    RabitQAttentionBackend)
                self.attn_backend = RabitQAttentionBackend
            else:
