...
def rabitq_int4_binary_scores(
    q_u: torch.Tensor,
    delta_vals: torch.Tensor,
    v_l_vals: torch.Tensor,
    sum_q_u: torch.Tensor,
    x_b: torch.Tensor,
    sum_x_b: torch.Tensor,
    key_norms: torch.Tensor,
    k_bar_dot_k: torch.Tensor,
    cq_dot_kr: torch.Tensor,
    q_norms: torch.Tensor,
    qr_dot_ck: torch.Tensor,
    cq_dot_ck: torch.Tensor,
    sqrt_head_size: float,
    denom_eps: float,
) -> torch.Tensor:
    return torch.ops._C.rabitq_int4_binary_scores(
        q_u, delta_vals, v_l_vals, sum_q_u, x_b, sum_x_b, key_norms,
        k_bar_dot_k, cq_dot_kr, q_norms, qr_dot_ck, cq_dot_ck,
        sqrt_head_size, denom_eps)


def rabitq_int4_packed_binary_scores(
    q_u: torch.Tensor,
    delta_vals: torch.Tensor,
    v_l_vals: torch.Tensor,
    sum_q_u: torch.Tensor,
    x_b_packed: torch.Tensor,  # [num_quantized, num_heads, head_size/32] uint32
    sum_x_b: torch.Tensor,
    key_norms: torch.Tensor,
    k_bar_dot_k: torch.Tensor,
    cq_dot_kr: torch.Tensor,
    ...
    """Packed Binary version with 8x memory bandwidth reduction for x_b."""
    return torch.ops._C.rabitq_int4_packed_binary_scores(
        q_u, delta_vals, v_l_vals, sum_q_u, x_b_packed, sum_x_b, key_norms,
        k_bar_dot_k, cq_dot_kr, q_norms, qr_dot_ck, cq_dot_ck, denom_eps)


def rabitq_int4_packed_binary_scores_warp(
    q_u: torch.Tensor,
    delta_vals: torch.Tensor,
    v_l_vals: torch.Tensor,
    sum_q_u: torch.Tensor,
    ...
    """Warp-level parallel version with warp shuffle reduction."""
    return torch.ops._C.rabitq_int4_packed_binary_scores_warp(
        q_u, delta_vals, v_l_vals, sum_q_u, x_b_packed, sum_x_b, key_norms,
        k_bar_dot_k, cq_dot_kr, q_norms, qr_dot_ck, cq_dot_ck, denom_eps)
...
def top_p_mask(
    ...
    """Compute top-p mask using ternary search (no sorting).

    Args:
        probs: Probability tensor of shape [batch_size, vocab_size] or similar.
        top_p: The cumulative probability threshold (0.0 to 1.0).

    Returns:
        A boolean mask tensor indicating which elements are within top-p.
    """
    return torch.ops._C.top_p_mask(probs, top_p)
