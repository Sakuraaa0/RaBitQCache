...
TORCH_LIBRARY_EXPAND(TORCH_EXTENSION_NAME, ops) {
  ...
  ops.def(
      "rabitq_int4_binary_scores("
      "Tensor q_u, Tensor delta_vals, Tensor v_l_vals, Tensor sum_q_u,"
      "Tensor x_b, Tensor sum_x_b, Tensor key_norms, Tensor k_bar_dot_k,"
      "Tensor cq_dot_kr, Tensor q_norms, Tensor qr_dot_ck, Tensor cq_dot_ck,"
      "float sqrt_head_size, float denom_eps) -> Tensor");
  ops.impl("rabitq_int4_binary_scores", torch::kCUDA,
           &rabitq_int4_binary_scores);

  // Packed Binary version: 8x memory bandwidth reduction for x_b
  ops.def(
      "rabitq_int4_packed_binary_scores("
      "Tensor q_u, Tensor delta_vals, Tensor v_l_vals, Tensor sum_q_u,"
      "Tensor x_b_packed, Tensor sum_x_b, Tensor key_norms, Tensor k_bar_dot_k,"
      "Tensor cq_dot_kr, Tensor q_norms, Tensor qr_dot_ck, Tensor cq_dot_ck,"
      "float denom_eps) -> Tensor");
  ops.impl("rabitq_int4_packed_binary_scores", torch::kCUDA,
           &rabitq_int4_packed_binary_scores);

  // Warp-level parallel version: uses warp shuffle for reduction
  ops.def(
      "rabitq_int4_packed_binary_scores_warp("
      "Tensor q_u, Tensor delta_vals, Tensor v_l_vals, Tensor sum_q_u,"
      "Tensor x_b_packed, Tensor sum_x_b, Tensor key_norms, Tensor k_bar_dot_k,"
      "Tensor cq_dot_kr, Tensor q_norms, Tensor qr_dot_ck, Tensor cq_dot_ck,"
      "float denom_eps) -> Tensor");
  ops.impl("rabitq_int4_packed_binary_scores_warp", torch::kCUDA,
           &rabitq_int4_packed_binary_scores_warp);

  // Top-P mask using ternary search (no sorting)
  ops.def("top_p_mask(Tensor probs, float top_p) -> Tensor");
  ops.impl("top_p_mask", torch::kCUDA, &top_p_mask);
